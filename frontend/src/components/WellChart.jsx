...
export default function WellChart({ logData, onDepthSelect, selectedDepth }) {
  ...
  // Normalise a curve to [0,1]
  const normalise = useCallback((key) => {
    const vals = curves[key].filter(v => v !== null && isFinite(v));
    const min = Math.min(...vals);
    const max = Math.max(...vals);
    const range = max - min || 1;
    return { min, max, norm: curves[key].map(v => v === null ? null : (v - min) / range) };
  }, [curves]);
  ...
  useEffect(() => {
    ...
    // Zone bands
    let prevZone = null;
    let zoneStart = depths[0];
    for (let i = 0; i < n; i++) {
      const z = zones[i];
      if (z !== prevZone || i === n - 1) {
        if (prevZone !== null) {
          const y1 = depthToY(zoneStart);
          const y2 = depthToY(depths[i]);
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      ctx.stroke();
      // Depth label
      const d = depths[0] + (i / 5) * (depths[n - 1] - depths[0]);
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    const idx = depths.reduce((best, d, i) => Math.abs(d - depth) < Math.abs(depths[best] - depth) ? i : best, 0);

    const vals = {};
    activeKeys.forEach(k => { if (curves[k]) vals[k] = curves[k][idx]; });
    setTooltip({ depth: depths[idx]?.toFixed(1), zone: zones[idx], vals, x: e.clientX - rect.left, y: e.clientY - rect.top });
    ...
    <div>
      ...
      <div className="log-toggles">
        {curve_names.map((key, i) => (
          <button
            key={key}
            className={`log-toggle ${activeKeys.includes(key) ? "active" : ""}`}
            style={{ borderColor: LOG_PALETTE[i % LOG_PALETTE.length], color: LOG_PALETTE[i % LOG_PALETTE.length] }}
      ...
      <div className="legend">
        {Object.entries({ oil: "#ca8a04", gas: "#dc2626", water: "#2563eb", shale: "#6b7280" }).map(([z, c]) => (
      ...
      <div ref={containerRef} style={{ position: "relative" }}>
        ...
        {tooltip && (
          <div style={{
            ...
            background: "#0f1117cc", border: "1px solid #2d3148",
            ...
            {Object.entries(tooltip.vals).map(([k, v]) => (
              <div key={k}>{k}: <strong>{v?.toFixed(2) ?? "N/A"}</strong></div>
            ))}
            <div style={{ color: "#64748b", marginTop: 4, fontSize: "0.72rem" }}>Klik et → AI izah etsin</div>
          </div>
        )}
      </div>
    </div>
