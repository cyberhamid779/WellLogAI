...
export default function ChatBox({ logData, selectedDepth, apiBase }) {
  const [messages, setMessages] = useState([
    { role: "ai", text: "Salam! Qrafikdəki istənilən dərinliyə klik et, mən həmin zona haqqında izah verim. Yaxud birbaşa sual yaz." }
  ]);
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${apiBase}/ask`, {
        question: q,
        depth: selectedDepth,
        log_data: {
  ...
  return (
    <div style={{ background: "#1a1d2e", borderRadius: 10, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ fontSize: "0.78rem", color: "#818cf8", fontWeight: 700, letterSpacing: 1 }}>💬 AI SUAL-CAVAB</div>
      ...
      <div style={{ maxHeight: 300, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {loading && (
          <div style={{ alignSelf: "flex-start", color: "#64748b", fontSize: "0.8rem" }}>Yazır...</div>
        )}
        <div ref={bottomRef} />
      </div>
